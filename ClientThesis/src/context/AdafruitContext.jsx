...
export const AdafruitProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        client.onMessageArrived = async (message) => {
            //update button status of relevant device
            console.log(message.payloadString)
            const controllerInfor = await fetchControllerInfo(message.payloadString)
            const options = {
            ...
            const vietnamTime = new Date(controllerInfor.data.created_at).toLocaleString('en-US', options)
            ...
            const controller = {
                ...
                value: parseInt(controllerInfor.data.value),
            }
            addControllersInfoToBlockchain([controller])
        ...
        const connect = () => {
            client.connect({
                useSSL: true,
                userName: aioUsername1,
                password: apiKey1,
                onSuccess: () => {
                    client.subscribe(`${aioUsername1}/feeds/relays.trigger`)
                },
                onFailure: (e) => {
                    console.log('Connection failed: ', e)
                },
